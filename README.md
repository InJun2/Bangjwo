...
- 결과적으로 Redis를 사용한 단순 락으로 문제를 해결하려고 하였는데, 그렇게 생각한 이유는 다음과 같습니다.
    ...
    - 현재 프로젝트의 웹 서버는 모놀리식 구조로 분산 환경을 고려할 필요는 없지만, 이후 `MSA`구조로 확장이 용이
    - 해당 이슈가 발생한 시점에서 이미 채팅, 로그인 리프레시토큰 관련으로 Redis를 사용하고 있음
...
- Redis SETNX와 Lua 스크립트를 활용하여 락의 획득부터 해제까지 원자성을 보장하는 분산락을 구현하여 동시성 문제를 해결했습니다.
 - **원자적 락 확인 및 획득(SETNX)** : Spring Data Redis의 setIfAbsent()를 활용해 Redis의 SETNX 명령어를 내부적으로 수행하도록 구현했습니다. 이를 통해 '락의 존재 여부 확인'과 '락 획득' 과정을 원자적으로 처리하여, 찰나의 순간에 여러 스레드가 동시에 접근하더라도 단 하나의 스레드만 락을 선점하도록 경쟁 조건을 원천 차단했습니다.
 - **안전한 락 해제 보장(Lua 스크립트)** : 락을 해제할 때 '현재 락의 소유자가 자신인지 확인'하고 '락을 제거'하는 두 과정이 분리되면, 그 사이에 TTL이 만료되어 다른 스레드가 획득한 락을 잘못 해제하는 치명적인 문제가 발생할 수 있습니다. 이를 방지하기 위해 Lua 스크립트를 도입, 두 연산을 Redis 내부에서 단일 명령처럼 원자적으로 실행되도록 묶어주어 락의 소유자만이 안전하게 락을 해제할 수 있도록 데이터 무결성을 확보했습니다.
 - **동시성 제어 검증** : `CountDownLatch`를 사용한 단위 테스트 코드를 작성하여 20개의 멀티스레드를 동시에 동작시켜 처음 하나의 요청만 성공하는 것을 확인해 동시성 문제를 해결했습니다.
